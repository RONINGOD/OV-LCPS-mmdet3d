...
class _PFCHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.pe_vision_proj = MLP(vision_clip_proj_channels,act_type='GELU',bias=False)
    ...
    def loss(self, batch_inputs, batch_data_samples, train_cfg):
        ...
        masked_voxel_features = self.mask_features(batch_inputs['features'].unsqueeze(0).transpose(1, 2)).squeeze(0).transpose(0,1)
        masked_voxel_features = self.pe_vision_proj(masked_voxel_features) # [V，256]
        class_preds_buffer, mask_preds_buffer, pos_mask_preds_buffer, sem_preds = self.forward(masked_voxel_features, batch_inputs['voxels']['voxel_coors'],text_features)
    ...
    def predict(self, batch_inputs, batch_data_samples):
        ...
        masked_voxel_features = self.mask_features(batch_inputs['features'].unsqueeze(0).transpose(1, 2)).squeeze(0).transpose(0,1)
        masked_voxel_features = self.pe_vision_proj(masked_voxel_features) # [V，256]
        class_preds_buffer, mask_preds_buffer, _, _ = self.forward(masked_voxel_features, batch_inputs['voxels']['voxel_coors'],text_features)
